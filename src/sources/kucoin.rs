...
impl KuCoinSource {
    ...
    async fn get_data_files(&self, config: &Config) -> Result<Vec<PathBuf>> {
        ...
        while current_date <= end_date {
            // KuCoin format: BTCUSDT-trades-2026-01-12.zip
            ...
            let url = format!(
                "https://historical-data.kucoin.com/data/spot/daily/trades/{}/{}",
                symbol, filename
            );
            ...
            match self.download_and_convert_to_parquet(&url, &cache_path).await {
                ...
                Err(_) => {
                    debug!("No KuCoin data found for {}", current_date.format("%Y-%m-%d"));
                }
            }
    ...
    fn process_csv_data(csv_data: &[u8]) -> Result<Vec<Tick>> {
        ...
        for result in reader.records() {
            ...
            // KuCoin format: trade_id,trade_time,price,size,side
            let timestamp: i64 = record[1].parse()?;
            let price: f64 = record[2].parse()?;
            let size: f64 = record[3].parse()?;
            let side = &record[4];
...
impl TickSource for KuCoinSource {
    async fn fetch_ticks(&self, config: &Config, tx: mpsc::Sender<Vec<Tick>>) -> Result<()> {
        info!("Fetching KuCoin data for {}{}", config.base, config.quote);
